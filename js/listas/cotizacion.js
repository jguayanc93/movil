...
const UIState = {
    ...
    container: document.getElementById('ncotis'),
...
function buildCardElement(data) {
    ...
    card.innerHTML = `
        ...
        <div class="p-4 sm:p-5">
            <div class="space-y-3">
                <div>
                    <p class="text-xs font-semibold text-indigo-600 uppercase tracking-wide">Cotización ID</p>
            ...
            <button class="mt-4 w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 rounded-lg transition-colors text-sm">
                Ver detalles
            </button>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    fetchAndDisplay(rutalistacoti);
});

document.getElementById('fecha-dia').addEventListener('change', () => {
    fetchAndDisplay(rutalistacotixdia, true);
});
